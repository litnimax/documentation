...
Nothing needs to be done! **Odoo sets up its own mail servers for the database.** Outgoing and
incoming emails work out-of-the-box!

Unless an external mail server is required to send large batches of mass mailing, simply enjoy the
new Odoo database.
...
com/trial>`_ & `Odoo.sh <https://www.odoo.sh>`_.
...
The following sections contain information on how to integrate an external email server with Odoo.
...
Can an Office 365 server be used
--------------------------------
...
To avoid that problem, make sure all emails use a "From" address from the authorized domain.
